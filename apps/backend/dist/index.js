...
const cookieConsent_1 = require("./routes/cookieConsent");
...
const introTemplate_1 = require("./routes/introTemplate");
const investor_1 = require("./routes/investor");
...
app.use('/api/introductions/templates', introTemplate_1.introTemplateRouter);
...
app.use('/api/cookie-consent', cookieConsent_1.cookieConsentRouter);
...
app.use('/api/webhooks', resendWebhook_1.webhookRouter);
app.use('/api/investors', investor_1.investorRouter);
if (env_1.env.SENTRY_DSN) {
...
server.listen(PORT, '0.0.0.0', async () => {
    ...
    const { processDripSequences } = await Promise.resolve().then(() => __importStar(require('./services/dripSequenceProcessor')));
    cron.default.schedule('0 * * * *', async () => {
        console.log('[Drip] Processing follow-up sequences...');
        await processDripSequences().catch(err => console.error('[Drip] Processing failed:', err));
