...
exports.aiBioRouter = void 0;
const express_1 = require("express");
const auth_1 = require("../middleware/auth");
const ai_1 = require("@cleya/ai");
const db_1 = require("@cleya/db");
exports.aiBioRouter = (0, express_1.Router)();
const PERSONA_LABEL = {
    FOUNDER: 'Founder',
    INVESTOR: 'Investor',
    TALENT: 'Talent',
...
exports.aiBioRouter.post('/bio', auth_1.authenticate, async (req, res, next) => {
    try {
        ...
        const user = await db_1.prisma.user.findUnique({ where: { id: userId }, select: { name: true } });
        const persona = profile?.persona ? PERSONA_LABEL[profile.persona] || profile.persona : null;
        const role = profile?.currentRole || null;
        const company = profile?.companyName || null;
        const stage = profile?.companyStage || null;
        ...
        const inputLines = [];
        if (user?.name)
            inputLines.push(`Name: ${user.name}`);
        if (persona)
            inputLines.push(`Persona: ${persona}`);
        if (role)
            inputLines.push(`Role: ${role}`);
        if (company)
            inputLines.push(`Company: ${company}`);
        if (stage)
            inputLines.push(`Company stage: ${stage}`);
        if (location)
            inputLines.push(`Location: ${location}`);
        if (industries.length)
            inputLines.push(`Industries: ${industries.slice(0, 6).join(', ')}`);
        if (skills.length)
            inputLines.push(`Skills: ${skills.slice(0, 8).join(', ')}`);
        if (lookingFor.length)
            inputLines.push(`Looking for: ${lookingFor.slice(0, 4).join(', ')}`);
        ...
        const ai = (0, ai_1.createAIService)({ provider: 'openai', model: 'gpt-4o-mini' });
