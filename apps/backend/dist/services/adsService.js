...
// Budget helpers
// ---------------------------------------------------------------------------
function enforceBudgetCap(budget) {
    return Math.min(budget, BUDGET_CAP_INR);
}
async function getTotalDailySpend() {
    const today = new Date().toISOString().split('T')[0];
    const logs = await (0, supabaseClient_1.supabaseSelect)('ad_audit_log', { action_date: today });
    if (!logs || logs.length === 0)
        return 0;
    return logs
        .filter((l) => l.action_type === 'budget_change' || l.action_type === 'campaign_created')
        .reduce((sum, l) => sum + (l.details?.budget_inr || 0), 0);
}
async function checkTotalSpendCap(additionalSpend) {
...
// Audit trail
// ---------------------------------------------------------------------------
async function logAuditEvent(platform, actionType, campaignId, details) {
    try {
        await (0, supabaseClient_1.supabaseInsert)('ad_audit_log', {
            platform,
...
async function createMetaCampaign(params) {
    ...
    try {
        const res = await fetch(`https://graph.facebook.com/${META_API_VERSION}/act_${META_AD_ACCOUNT_ID}/campaigns`, {
            method: 'POST',
...
async function createMetaAdSet(params) {
    ...
    try {
        const res = await fetch(`https://graph.facebook.com/${META_API_VERSION}/act_${META_AD_ACCOUNT_ID}/adsets`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                access_token: META_ACCESS_TOKEN,
...
async function createMetaAd(params) {
    ...
    try {
        ...
        const creativeRes = await fetch(`https://graph.facebook.com/${META_API_VERSION}/act_${META_AD_ACCOUNT_ID}/adcreatives`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                access_token: META_ACCESS_TOKEN,
        ...
        if (creativeData.error)
            return { success: false, error: creativeData.error.message, data: creativeData };
        // Create the ad using the creative
        const adRes = await fetch(`https://graph.facebook.com/${META_API_VERSION}/act_${META_AD_ACCOUNT_ID}/ads`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                access_token: META_ACCESS_TOKEN,
        ...
        if (adData.error)
            return { success: false, error: adData.error.message, data: adData };
...
async function activateMetaCampaign(campaignId) {
    if (!META_ACCESS_TOKEN)
        return { success: false, error: 'Meta Ads not configured.' };
    // Verify campaign exists and check hold period
    try {
        const infoRes = await fetch(`https://graph.facebook.com/${META_API_VERSION}/${campaignId}?fields=id,name,status,created_time,adsets{id,name,targeting,ads{id,name}}&access_token=${META_ACCESS_TOKEN}`);
        const info = await infoRes.json();
        if (info.error)
            return { success: false, error: info.error.message };
        ...
        if (adsets.length === 0)
            return { success: false, error: 'Cannot activate: campaign has no ad sets.' };
        const hasAds = adsets.some((as) => as.ads?.data?.length > 0);
        if (!hasAds)
            return { success: false, error: 'Cannot activate: no ad creatives in any ad set.' };
...
async function pauseMetaCampaign(campaignId) {
    if (!META_ACCESS_TOKEN)
        return { success: false, error: 'Meta Ads not configured.' };
    try {
        const res = await fetch(`https://graph.facebook.com/${META_API_VERSION}/${campaignId}`, {
...
async function updateMetaBudget(campaignId, newBudget) {
    if (!META_ACCESS_TOKEN)
        return { success: false, error: 'Meta Ads not configured.' };
...
async function getMetaCampaigns() {
    ...
    try {
        const res = await fetch(`https://graph.facebook.com/${META_API_VERSION}/act_${META_AD_ACCOUNT_ID}/campaigns?fields=id,name,status,daily_budget,objective,insights{impressions,clicks,spend,cpc,cpm,ctr}&access_token=${META_ACCESS_TOKEN}`);
        const data = await res.json();
        if (data.error)
            return { success: false, error: data.error.message, data };
...
async function getAdSetInsights(adSetId, datePreset) {
    if (!META_ACCESS_TOKEN)
        return { success: false, error: 'Meta Ads not configured.' };
    try {
        const preset = datePreset || 'last_7d';
        const res = await fetch(`https://graph.facebook.com/${META_API_VERSION}/${adSetId}/insights?fields=impressions,clicks,spend,cpc,cpm,ctr,reach,frequency,actions,cost_per_action_type,conversions&date_preset=${preset}&access_token=${META_ACCESS_TOKEN}`);
        const data = await res.json();
        if (data.error)
            return { success: false, error: data.error.message, data };
        return { success: true, data: data.data || [] };
...
async function getCampaignInsights(campaignId, datePreset) {
    if (!META_ACCESS_TOKEN)
        return { success: false, error: 'Meta Ads not configured.' };
    try {
        const preset = datePreset || 'last_7d';
        const res = await fetch(`https://graph.facebook.com/${META_API_VERSION}/${campaignId}/insights?fields=impressions,clicks,spend,cpc,cpm,ctr,reach,frequency,actions,cost_per_action_type,conversions,cost_per_conversion&date_preset=${preset}&access_token=${META_ACCESS_TOKEN}`);
        const data = await res.json();
        if (data.error)
            return { success: false, error: data.error.message, data };
        return { success: true, data: data.data || [] };
...
async function googleAdsRequest(method, path, body) {
...
async function createGoogleCampaign(params) {
    ...
    try {
        ...
        if (!budgetResourceName)
            return { success: false, error: 'Failed to create Google Ads budget' };
        ...
        if (!campaignResourceName)
            return { success: false, error: 'Failed to create Google Ads campaign' };
...
async function getGoogleCampaignPerformance(dateRange) {
    if (!GOOGLE_ADS_DEVELOPER_TOKEN || !GOOGLE_ADS_CUSTOMER_ID) {
        return { success: false, error: 'Google Ads not configured.' };
    }
    try {
    ...
    `;
        ...
        const campaigns = (data || []).flatMap((batch) => (batch.results || []).map((r) => ({
            id: r.campaign?.id,
...
async function getGoogleCampaigns() {
    return getGoogleCampaignPerformance();
}
async function activateGoogleCampaign(campaignResourceName) {
    if (!GOOGLE_ADS_DEVELOPER_TOKEN || !GOOGLE_ADS_CUSTOMER_ID) {
        return { success: false, error: 'Google Ads not configured.' };
    }
    try {
...
async function pauseGoogleCampaign(campaignResourceName) {
    if (!GOOGLE_ADS_DEVELOPER_TOKEN || !GOOGLE_ADS_CUSTOMER_ID) {
        return { success: false, error: 'Google Ads not configured.' };
    }
    try {
...
async function updateGoogleBudget(budgetResourceName, newBudget) {
    if (!GOOGLE_ADS_DEVELOPER_TOKEN || !GOOGLE_ADS_CUSTOMER_ID) {
        return { success: false, error: 'Google Ads not configured.' };
    }
    const budget = enforceBudgetCap(newBudget);
...
async function createLinkedInCampaign(params) {
...
async function activateLinkedInCampaign(campaignId) {
    if (!LINKEDIN_ADS_TOKEN)
        return { success: false, error: 'LinkedIn Ads not configured.' };
    try {
...
async function pauseLinkedInCampaign(campaignId) {
    if (!LINKEDIN_ADS_TOKEN)
        return { success: false, error: 'LinkedIn Ads not configured.' };
    try {
        const res = await fetch(`https://api.linkedin.com/rest/adCampaigns/${campaignId}`, {
...
async function getLinkedInCampaigns() {
    ...
    try {
        const res = await fetch(`https://api.linkedin.com/rest/adCampaigns?q=search&search=(account:(values:List(urn:li:sponsoredAccount:${LINKEDIN_AD_ACCOUNT_ID})))`, { headers: LINKEDIN_HEADERS });
        if (!res.ok) {
            const errText = await res.text();
            return { success: false, error: `LinkedIn Ads API error: ${res.status} - ${errText}` };
        }
...
async function getLinkedInCampaignAnalytics(campaignIds, dateRange) {
    if (!LINKEDIN_ADS_TOKEN || !LINKEDIN_AD_ACCOUNT_ID) {
        return { success: false, error: 'LinkedIn Ads not configured.' };
    }
    try {
...
async function getCampaigns(platform) {
    switch (platform) {
        case 'meta':
...
async function activateCampaign(platform, campaignId) {
    switch (platform) {
        case 'meta':
...
async function pauseCampaign(platform, campaignId) {
    switch (platform) {
        case 'meta':
...
async function updateBudget(platform, resourceId, newBudget) {
    switch (platform) {
        case 'meta':
...
async function optimizeCampaigns(platform) {
