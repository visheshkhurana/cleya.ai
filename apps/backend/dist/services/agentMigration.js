...
exports.ensureAgentTables = ensureAgentTables;
...
async function ensureAgentTables() {
    ...
    await (0, founderModeService_1.ensureFounderModeTables)();
...
async function ensureAgentMessagesTables() {
    try {
        await db_1.prisma.$executeRawUnsafe(`
...
async function ensureContentCalendarTables() {
    try {
        await db_1.prisma.$executeRawUnsafe(`
...
async function ensureAuditAndSupportTables() {
    try {
        await db_1.prisma.$executeRawUnsafe(`
...
async function ensureCampaignMetricsTables() {
    try {
        await db_1.prisma.$executeRawUnsafe(`
...
async function seedOutreachAgent() {
    try {
        const { ensureOutreachTables } = await Promise.resolve().then(() => __importStar(require('./outreachCampaignService')));
...
async function ensureQAReportsTables() {
    try {
        await db_1.prisma.$executeRawUnsafe(`
...
async function ensureAutonomousWorkflowTables() {
...
async function ensureSEOReportsTables() {
    try {
        await db_1.prisma.$executeRawUnsafe(`
...
async function ensureAgentChatHistoryTable() {
    try {
        await db_1.prisma.$executeRawUnsafe(`
...
async function ensureSharedMemoryTable() {
    try {
        await db_1.prisma.$executeRawUnsafe(`
    ...
    `);
        await db_1.prisma.$executeRawUnsafe(`CREATE INDEX IF NOT EXISTS idx_shared_memory_category ON dm_shared_memory(category);`);
        await db_1.prisma.$executeRawUnsafe(`CREATE INDEX IF NOT EXISTS idx_shared_memory_author ON dm_shared_memory(author_agent_id);`);
        await db_1.prisma.$executeRawUnsafe(`CREATE INDEX IF NOT EXISTS idx_shared_memory_importance ON dm_shared_memory(importance);`);
...
async function ensureAgentCommsTable() {
    try {
        await db_1.prisma.$executeRawUnsafe(`
    ...
    `);
        await db_1.prisma.$executeRawUnsafe(`CREATE INDEX IF NOT EXISTS idx_agent_comms_to ON dm_agent_comms(to_agent_id, status, created_at DESC);`);
        await db_1.prisma.$executeRawUnsafe(`CREATE INDEX IF NOT EXISTS idx_agent_comms_from ON dm_agent_comms(from_agent_id, created_at DESC);`);
        await db_1.prisma.$executeRawUnsafe(`CREATE INDEX IF NOT EXISTS idx_agent_comms_type ON dm_agent_comms(message_type);`);
...
async function ensureFilesTable() {
    ...
    `);
        await db_1.prisma.$executeRawUnsafe(`CREATE INDEX IF NOT EXISTS idx_files_file_id ON dm_files(file_id);`);
        await db_1.prisma.$executeRawUnsafe(`CREATE INDEX IF NOT EXISTS idx_files_created_by ON dm_files(created_by, created_at DESC);`);
        await db_1.prisma.$executeRawUnsafe(`CREATE INDEX IF NOT EXISTS idx_files_category ON dm_files(category);`);
...
//# sourceMappingURL=agentMigration.js.map