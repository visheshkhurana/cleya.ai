...
exports.notifyAgentCompletion = notifyAgentCompletion;
exports.sendBriefingNotifications = sendBriefingNotifications;
...
async function notifyAgentCompletion(notification) {
    const promises = [];
...
async function notifyViaSlack(notification) {
    try {
...
async function notifyViaEmail(notification) {
    try {
...
async function sendBriefingNotifications(briefing) {
    const promises = [];
    promises.push(sendBriefingSlack(briefing));
    promises.push(sendBriefingEmail(briefing));
    await Promise.allSettled(promises);
}
async function sendBriefingSlack(briefing) {
    try {
        ...
        await slackService_1.slackService.postAlert(`Daily Nexus Briefing — ${briefing.date}`, blocks);
    }
    catch (err) {
...
//# sourceMappingURL=agentNotifier.js.map