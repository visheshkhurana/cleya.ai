...
exports.TOOL_DEFINITIONS = exports.AGENT_TOOLS = void 0;
exports.getToolsForAgent = getToolsForAgent;
exports.getOpenAIToolSchemas = getOpenAIToolSchemas;
...
exports.TOOL_DEFINITIONS = {
    ...
    launch_campaign: {
        ...
        description: 'Launch a full ad campaign (campaign + ad set + creatives) across selected platforms in one shot. Creates everything PAUSED with a 1-hour activation hold. Targets Cleya audiences (founders, investors, operators) in India. For email outreach campaigns, pass campaignId instead.',
        parameters: {
            type: 'object',
            properties: {
                objective: { type: 'string', description: 'Ad campaign objective (e.g. BRAND_AWARENESS, LEAD_GENERATION, CONVERSIONS, TRAFFIC)' },
                ...
                campaignId: { type: 'string', description: 'For email outreach: the campaign ID to launch' },
            },
            required: [],
        },
    },
    ...
    pause_campaign: {
        name: 'pause_campaign',
        description: 'Pause an active ad campaign or email outreach campaign. For ad campaigns, provide platform + campaignId. For email outreach, provide campaignId only.',
        parameters: {
            type: 'object',
            properties: {
                platform: { type: 'string', enum: ['meta', 'linkedin', 'google'], description: 'Ad platform (omit for email outreach campaigns)' },
...
function getToolsForAgent(agentId) {
    const toolNames = exports.AGENT_TOOLS[agentId];
    if (!toolNames)
        return [];
    return toolNames.map(name => exports.TOOL_DEFINITIONS[name]).filter(Boolean);
}
function getOpenAIToolSchemas(agentId) {
...
async function executeTool(agentId, toolName, params) {
    ...
    const allowedTools = exports.AGENT_TOOLS[agentId];
    ...
    if (actionTools.includes(toolName)) {
        ...
        const riskAssessment = (0, guardrailsService_1.scoreContentRisk)(contentToCheck, toolName);
        const guardrailCheck = await (0, guardrailsService_1.checkGuardrails)(agentId, toolName, guardrails, contentToCheck);
    ...
    try {
        ...
        switch (toolName) {
            case 'post_to_social':
                result = await ayrshareService_1.ayrshareService.post({
                ...
                break;
            case 'schedule_post':
                result = await ayrshareService_1.ayrshareService.schedulePost({
                    content: params.content,
                    platforms: params.platforms,
            ...
            case 'get_post_analytics':
                result = await ayrshareService_1.ayrshareService.getAnalytics({ postId: params.postId });
                break;
            case 'get_post_history':
                result = await ayrshareService_1.ayrshareService.getHistory();
                break;
            case 'create_ad_campaign':
                result = await adsService_1.adsService.createCampaign({
            ...
            case 'get_campaign_stats':
                result = await adsService_1.adsService.getCampaigns(params.platform);
                break;
            case 'optimize_campaigns':
                result = await adsService_1.adsService.optimizeCampaigns(params.platform);
                break;
            case 'activate_campaign':
                result = await adsService_1.adsService.activateCampaign(params.platform, params.campaignId);
                break;
            case 'adjust_budget':
                result = await adsService_1.adsService.updateBudget(params.platform, params.resourceId, params.newBudget);
                break;
            case 'generate_ad_copy': {
                const copy = await adCampaignAutomation_1.adCampaignAutomation.generateAdCopy(params.product, params.audience, params.platform);
            ...
            case 'get_campaign_report': {
                const dateRange = params.startDate && params.endDate
                    ...
                    : undefined;
                const report = await adCampaignAutomation_1.adCampaignAutomation.generateCampaignReport(dateRange);
            ...
            case 'create_email_campaign': {
                const { createCampaign } = await Promise.resolve().then(() => __importStar(require('./outreachCampaignService')));
                result = await createCampaign(params);
                break;
            }
            case 'add_recipients': {
                const { addRecipients } = await Promise.resolve().then(() => __importStar(require('./outreachCampaignService')));
                result = await addRecipients(params);
                break;
            }
            case 'launch_campaign': {
                if (params.platforms && Array.isArray(params.platforms)) {
                    // Ad campaign launch (multi-platform)
                    const strategy = await adCampaignAutomation_1.adCampaignAutomation.generateCampaignStrategy(params.objective, params.totalBudget, params.platforms);
                    const launchResult = await adCampaignAutomation_1.adCampaignAutomation.launchCampaign(strategy);
                    result = { success: launchResult.success, data: launchResult };
                }
                else {
                    // Email outreach campaign launch
                    const { launchCampaign } = await Promise.resolve().then(() => __importStar(require('./outreachCampaignService')));
                    result = await launchCampaign(params.campaignId);
                }
                break;
            }
            case 'get_campaign_analytics': {
                const { getCampaignStats } = await Promise.resolve().then(() => __importStar(require('./outreachCampaignService')));
                result = await getCampaignStats(params.campaignId);
                break;
            }
            case 'get_recipient_list': {
                const { getRecipientList } = await Promise.resolve().then(() => __importStar(require('./outreachCampaignService')));
                result = await getRecipientList(params.campaignId, params.status);
                break;
            }
            case 'pause_campaign': {
                if (params.platform) {
                    // Ad campaign pause (platform-specific)
                    result = await adsService_1.adsService.pauseCampaign(params.platform, params.campaignId);
                }
                else {
                    // Email outreach campaign pause
                    const { pauseCampaign } = await Promise.resolve().then(() => __importStar(require('./outreachCampaignService')));
            ...
            // --- Probe QA tools ---
            case 'run_page_test': {
                const { runSinglePageTest } = await Promise.resolve().then(() => __importStar(require('./qaAutomation')));
                result = await runSinglePageTest(params.url);
                break;
            }
            case 'run_api_test': {
                const { runSingleAPITest } = await Promise.resolve().then(() => __importStar(require('./qaAutomation')));
                result = await runSingleAPITest(params.url, params.method || 'GET');
                break;
            }
            case 'run_agent_test': {
                const { runSingleAgentTest } = await Promise.resolve().then(() => __importStar(require('./qaAutomation')));
                result = await runSingleAgentTest(params.agentId);
                break;
            }
            case 'run_full_qa': {
                const { runDailyQARoutine } = await Promise.resolve().then(() => __importStar(require('./qaAutomation')));
                result = await runDailyQARoutine();
                break;
            }
            case 'get_qa_report': {
                if (params.date) {
                    const { getQAReportByDate } = await Promise.resolve().then(() => __importStar(require('./qaAutomation')));
                    result = await getQAReportByDate(params.date);
                }
                else {
                    const { getLatestQAReport } = await Promise.resolve().then(() => __importStar(require('./qaAutomation')));
            ...
            case 'get_analytics_summary':
                result = await (0, analyticsService_1.getAnalyticsSummary)();
                break;
            case 'get_top_pages': {
                const startDate = params.startDate || '7daysAgo';
                const endDate = params.endDate || 'today';
                const limit = params.limit || 10;
                result = await (0, analyticsService_1.getTopPages)(startDate, endDate, limit);
                break;
            }
            case 'get_traffic_sources': {
                const startDate = params.startDate || '7daysAgo';
                const endDate = params.endDate || 'today';
                result = await (0, analyticsService_1.getTrafficSources)(startDate, endDate);
                break;
            }
            default:
        ...
        await (0, guardrailsService_1.logExecution)({
            ...
            autonomyLevel: (0, agentRunner_1.getAgentAutonomyLevel)(agentId),
...
function extractMetaTags(html) {
    ...
    while ((match = metaRegexReversed.exec(html)))
...
function findStructuredData(html) {
    const schemas = [];
...
async function executeAnalyzeSeo(url) {
    try {
        ...
        if (!metaTags['title'])
            ...
            issues.push(`Title too long (${metaTags['title'].length} chars, recommended ≤60)`);
        else if (metaTags['title'].length < 30)
            issues.push(`Title too short (${metaTags['title'].length} chars, recommended 30-60)`);
        if (!metaTags['description'])
            issues.push('Missing meta description');
        else if (metaTags['description'].length > 160)
            issues.push(`Meta description too long (${metaTags['description'].length} chars, recommended ≤160)`);
...
async function executeKeywordResearch(topic, count) {
    ...
    const suggestions = keywordCategories.flatMap(cat => {
        ...
        if (cat.category === 'Primary') {
            keywords.push({ keyword: base, category: cat.category, intent: 'informational', difficulty: 'medium' }, { keyword: `best ${base}`, category: cat.category, intent: 'commercial', difficulty: 'medium' }, { keyword: `${base} platform`, category: cat.category, intent: 'commercial', difficulty: 'low' }, { keyword: `${base} 2024`, category: cat.category, intent: 'informational', difficulty: 'low' });
        }
        else if (cat.category === 'Long-tail') {
            keywords.push({ keyword: `how to find ${base}`, category: cat.category, intent: 'informational', difficulty: 'low' }, { keyword: `${base} for startups`, category: cat.category, intent: 'commercial', difficulty: 'low' }, { keyword: `${base} app for founders`, category: cat.category, intent: 'transactional', difficulty: 'low' }, { keyword: `top ${base} tools`, category: cat.category, intent: 'commercial', difficulty: 'medium' });
        }
        else if (cat.category === 'Question-based') {
            keywords.push({ keyword: `what is ${base}`, category: cat.category, intent: 'informational', difficulty: 'low' }, { keyword: `how does ${base} work`, category: cat.category, intent: 'informational', difficulty: 'low' }, { keyword: `why use ${base}`, category: cat.category, intent: 'informational', difficulty: 'low' }, { keyword: `is ${base} worth it`, category: cat.category, intent: 'commercial', difficulty: 'low' });
        }
        else if (cat.category === 'Local') {
            keywords.push({ keyword: `${base} in Bangalore`, category: cat.category, intent: 'local', difficulty: 'low' }, { keyword: `${base} in Delhi NCR`, category: cat.category, intent: 'local', difficulty: 'low' }, { keyword: `${base} in Mumbai`, category: cat.category, intent: 'local', difficulty: 'low' }, { keyword: `${base} India`, category: cat.category, intent: 'local', difficulty: 'medium' });
        }
        else if (cat.category === 'GEO-optimized') {
            keywords.push({ keyword: `${base} — complete guide`, category: cat.category, intent: 'informational', difficulty: 'medium' }, { keyword: `${base} statistics and data`, category: cat.category, intent: 'informational', difficulty: 'low' }, { keyword: `${base} comparison`, category: cat.category, intent: 'commercial', difficulty: 'medium' }, { keyword: `${base} expert recommendations`, category: cat.category, intent: 'informational', difficulty: 'low' });
        }
        return keywords;
    });
    return {
...
async function executeAnalyzeCompetitors(urls) {
    const results = await Promise.all(urls.slice(0, 5).map(async (url) => {
        try {
            ...
            if (metaTags['description'] && metaTags['description'].length >= 120 && metaTags['description'].length <= 160)
                strengths.push('Good meta description length');
            else
                weaknesses.push('Meta description not optimized');
            if (metaTags['og:title'] && metaTags['og:image'])
                strengths.push('Open Graph tags present');
            else
                weaknesses.push('Missing Open Graph tags');
            if (structuredData.length > 0)
                strengths.push(`Has ${structuredData.length} structured data schema(s)`);
            else
                weaknesses.push('No structured data');
            if (headings['h1']?.length === 1)
                strengths.push('Single H1 tag (good practice)');
            else
                weaknesses.push('H1 tag issues');
            if (wordCount > 1000)
                strengths.push(`Good content depth (${wordCount} words)`);
            else
                weaknesses.push(`Thin content (${wordCount} words)`);
            return {
                url,
                statusCode,
                ...
                structuredDataTypes: structuredData.map((s) => s['@type'] || 'unknown'),
                wordCount,
                strengths,
                weaknesses,
            };
...
function executeGenerateSchemaMarkup(schemaType, data) {
    let schema;
    switch (schemaType) {
        case 'Organization':
            schema = {
...
async function executeCheckIndexing(urls) {
...
async function executeOptimizeContent(content, targetKeyword, contentType) {
