...
exports.ensureSafetyTables = ensureSafetyTables;
exports.notifyApprovalNeeded = notifyApprovalNeeded;
exports.processEscalations = processEscalations;
exports.getCrisisMode = getCrisisMode;
exports.activateCrisisMode = activateCrisisMode;
exports.deactivateCrisisMode = deactivateCrisisMode;
exports.isCrisisModeActive = isCrisisModeActive;
exports.sendDailyAuditDigest = sendDailyAuditDigest;
...
async function notifyApprovalNeeded(decision) {
    try {
...
async function processEscalations() {
    ...
    let autoCancelled = 0;
    try {
        const pendingDecisions = await (0, supabaseClient_1.supabaseSelect)('dm_decisions', { status: 'pending' });
        const now = Date.now();
        for (const decision of pendingDecisions) {
            const createdAt = new Date(decision.created_at).getTime();
            const hoursElapsed = (now - createdAt) / (1000 * 60 * 60);
            if (hoursElapsed >= AUTO_CANCEL_HOURS) {
                await (0, supabaseClient_1.supabaseUpdate)('dm_decisions', { id: String(decision.id) }, {
                    status: 'rejected',
                    founder_notes: `Auto-cancelled after ${AUTO_CANCEL_HOURS} hours with no response`,
                    decided_at: new Date().toISOString(),
                });
                try {
                    await (0, supabaseClient_1.supabaseInsert)('dm_execution_log', {
                        agent_id: 'system',
                ...
                await slackService_1.slackService.postAlert(`Decision auto-cancelled: ${decision.title}`, blocks);
                autoCancelled++;
            }
                ...
                if (!alreadyEscalated) {
                    ...
                    await slackService_1.slackService.postAlert(`ESCALATION: Decision pending ${Math.round(hoursElapsed)}h - ${decision.title}`, blocks);
                    escalated++;
                }
            }
        }
...
async function getCrisisMode() {
    try {
        const rows = await db_1.prisma.$queryRawUnsafe(`SELECT value FROM dm_system_flags WHERE key = 'crisis_mode'`);
...
async function activateCrisisMode(activatedBy, reason) {
    const state = {
        active: true,
    ...
    try {
        await db_1.prisma.$executeRawUnsafe(`INSERT INTO dm_system_flags (key, value, updated_at)
       VALUES ('crisis_mode', $1::jsonb, now())
       ON CONFLICT (key) DO UPDATE SET value = $1::jsonb, updated_at = now()`, JSON.stringify(state));
    }
    catch (err) {
    ...
    await slackService_1.slackService.postAlert('CRISIS MODE ACTIVATED - All autonomous activity paused', blocks);
    console.log(`[FounderSafety] Crisis mode activated by ${activatedBy}: ${reason}`);
    return state;
}
async function deactivateCrisisMode(deactivatedBy) {
    const state = {
        active: false,
        activatedAt: null,
        activatedBy: null,
    ...
    await slackService_1.slackService.postAlert('Crisis mode deactivated - Normal operations resumed', blocks);
    console.log(`[FounderSafety] Crisis mode deactivated by ${deactivatedBy}`);
    return state;
}
async function isCrisisModeActive() {
    const state = await getCrisisMode();
    return state.active;
}
async function sendDailyAuditDigest() {
    const todayStart = new Date();
    todayStart.setHours(0, 0, 0, 0);
    let executionLogs = [];
    try {
        const allLogs = await (0, supabaseClient_1.supabaseSelect)('dm_execution_log', undefined, {
            order: 'executed_at.desc',
            limit: 500,
        });
        executionLogs = allLogs.filter((l) => new Date(l.executed_at || l.created_at) >= todayStart);
    }
    catch { }
    let agentLogs = [];
    try {
        const allAgentLogs = await (0, supabaseClient_1.supabaseSelect)('dm_agent_logs', undefined, {
            order: 'created_at.desc',
            limit: 500,
        });
        agentLogs = allAgentLogs.filter((l) => new Date(l.created_at) >= todayStart);
    }
    catch { }
    const agentSummary = {};
    for (const log of executionLogs) {
        ...
        if (log.execution_result === 'success')
            agentSummary[agentId].taken++;
        else if (log.execution_result === 'error')
            agentSummary[agentId].errors++;
        else if (log.guardrail_result === 'blocked')
            agentSummary[agentId].blocked++;
        else if (log.execution_result === 'queued')
            agentSummary[agentId].pending++;
    }
    ...
    const totalSpend = executionLogs.reduce((sum, l) => sum + (parseFloat(l.spend_amount) || 0), 0);
    let pendingDecisions = 0;
    try {
        const pending = await (0, supabaseClient_1.supabaseSelect)('dm_decisions', { status: 'pending' });
    ...
    await slackService_1.slackService.postAlert(`Daily Audit Digest — ${dateStr}`, blocks);
    try {
...
async function sendWeeklyPerformanceReport() {
    ...
    const [totalUsers, newUsersThisWeek, completedProfiles, totalMatches, acceptedMatches, totalCalls, totalMessages,] = await Promise.all([
        db_1.prisma.user.count(),
        db_1.prisma.user.count({ where: { createdAt: { gte: weekStart } } }),
        db_1.prisma.profile.count({ where: { isComplete: true } }),
        db_1.prisma.match.count({ where: { createdAt: { gte: weekStart } } }),
        db_1.prisma.match.count({ where: { status: 'ACCEPTED', createdAt: { gte: weekStart } } }),
        db_1.prisma.call.count({ where: { createdAt: { gte: weekStart } } }),
        db_1.prisma.messageRecord.count({ where: { createdAt: { gte: weekStart } } }),
    ]);
    let contentPublished = 0;
    let contentPending = 0;
    try {
        const published = await (0, supabaseClient_1.supabaseSelect)('dm_content_queue', { status: 'published' });
        const publishedThisWeek = published.filter((c) => new Date(c.created_at) >= weekStart);
        contentPublished = publishedThisWeek.length;
        const pending = await (0, supabaseClient_1.supabaseSelect)('dm_content_queue', { status: 'pending' });
    ...
    try {
        ...
        executionLogs = allLogs.filter((l) => new Date(l.executed_at || l.created_at) >= weekStart);
    }
    catch { }
...
//# sourceMappingURL=founderSafetyService.js.map