...
exports.DEFAULT_GUARDRAILS = void 0;
exports.scoreContentRisk = scoreContentRisk;
exports.classifyActionRisk = classifyActionRisk;
exports.checkGuardrails = checkGuardrails;
exports.shouldAutoExecute = shouldAutoExecute;
...
function scoreContentRisk(content, actionType) {
    const factors = [];
    let score = 1;
    let financialHits = 0;
...
function classifyActionRisk(actionType) {
    const riskAssessment = scoreContentRisk('', actionType);
    return riskAssessment.score >= 3 ? 'high' : 'low';
}
async function checkGuardrails(agentId, actionType, guardrails, content, spendAmount) {
    ...
    try {
        const todayLogs = await (0, supabaseClient_1.supabaseSelect)('dm_execution_log', {
            agent_id: agentId,
        });
        const todayActions = todayLogs.filter((l) => new Date(l.executed_at) >= todayStart);
        ...
        const publishActions = todayLogs.filter((l) => new Date(l.executed_at) >= todayStart &&
        ...
        if (spendAmount && spendAmount > 0) {
            const todaySpend = todayLogs
                .filter((l) => new Date(l.executed_at) >= todayStart && l.spend_amount)
                .reduce((sum, l) => sum + (l.spend_amount || 0), 0);
...
function shouldAutoExecute(autonomyLevel, riskScore) {
...
async function logAudit(params) {
    try {
        await (0, supabaseClient_1.supabaseInsert)('audit_log', {
            agent_id: params.agentId,
            action_type: params.actionType,
            action_description: params.actionDescription,
...
//# sourceMappingURL=guardrailsService.js.map