...
exports.USD_TO_INR = exports.HIGH_COST_THRESHOLD_INR = void 0;
exports.routeModel = routeModel;
exports.inferTaskType = inferTaskType;
exports.isHighCostTask = isHighCostTask;
...
function routeModel(input) {
    ...
    if (language && INDIAN_LANGUAGES.includes(language.toLowerCase())) {
        ...
        return {
            ...
            primaryProvider: (0, ai_1.getProviderForModel)(override.primary),
            fallbackModel: override.fallback,
            fallbackProvider: (0, ai_1.getProviderForModel)(override.fallback),
    ...
    if (taskType && TASK_TYPE_OVERRIDES[taskType]) {
        ...
        return {
            ...
            primaryProvider: (0, ai_1.getProviderForModel)(primary),
            fallbackModel: fallback,
            fallbackProvider: (0, ai_1.getProviderForModel)(fallback),
            reason: override.reason,
        };
    }
...
function inferTaskType(agentId, taskContext) {
    if (!taskContext) {
        const agentTaskMap = {
    ...
    if (lower.includes('linkedin') && (lower.includes('thought leadership') || lower.includes('article')))
        return 'thought_leadership';
    if (lower.includes('linkedin'))
        return 'linkedin_post';
    if (lower.includes('instagram') || lower.includes('insta'))
        return 'instagram_post';
    if (lower.includes('caption') || lower.includes('tweet') || lower.includes('social'))
        return 'social_caption';
    if (lower.includes('newsletter') || lower.includes('email campaign'))
        return 'newsletter';
    if (lower.includes('blog'))
        return 'blog_article';
    if (lower.includes('support') && lower.includes('reply'))
        return 'support_reply';
    if (lower.includes('faq'))
        return 'support_faq';
    if (lower.includes('financial') || lower.includes('runway') || lower.includes('burn rate'))
    ...
    for (const lang of INDIAN_LANGUAGES) {
        if (lower.includes(lang))
            return 'indian_language_content';
    }
    return inferTaskType(agentId);
}
exports.HIGH_COST_THRESHOLD_INR = 500;
exports.USD_TO_INR = 85;
function isHighCostTask(costUSD) {
    return costUSD * exports.USD_TO_INR >= exports.HIGH_COST_THRESHOLD_INR;
}
