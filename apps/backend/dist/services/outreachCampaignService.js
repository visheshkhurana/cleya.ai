...
exports.ensureOutreachTables = ensureOutreachTables;
...
async function ensureOutreachTables() {
    ...
    `);
        await db_1.prisma.$executeRawUnsafe(`CREATE INDEX IF NOT EXISTS idx_outreach_campaigns_status ON outreach_campaigns(status);`);
        await db_1.prisma.$executeRawUnsafe(`CREATE INDEX IF NOT EXISTS idx_outreach_campaigns_id ON outreach_campaigns(campaign_id);`);
        await db_1.prisma.$executeRawUnsafe(`
    ...
    `);
        await db_1.prisma.$executeRawUnsafe(`CREATE INDEX IF NOT EXISTS idx_outreach_unsubscribes_email ON outreach_unsubscribes(email);`);
...
async function createCampaign(params) {
    ...
    `, campaignId, params.name, params.subjectLine, params.emailBody, params.targetSegment || 'all', JSON.stringify(params.targetFilters || {}), JSON.stringify(steps), params.scheduledAt || null);
        return {
            success: true,
            campaignId,
            name: params.name,
...
async function addRecipients(params) {
    try {
        const campaigns = await db_1.prisma.$queryRawUnsafe(`SELECT campaign_id FROM outreach_campaigns WHERE campaign_id = $1`, params.campaignId);
        if (!campaigns.length)
            return { success: false, error: 'Campaign not found. Create a campaign first.' };
        const existingCount = await db_1.prisma.$queryRawUnsafe(`SELECT COUNT(*)::int as count FROM outreach_recipients WHERE campaign_id = $1`, params.campaignId);
        ...
        `, params.campaignId, r.email.toLowerCase().trim(), r.firstName || '', r.lastName || '', r.company || '', r.role || '', JSON.stringify(r.personalization || {}));
                added++;
            }
            catch {
                skippedDupe++;
...
async function launchCampaign(campaignId) {
    try {
        const campaigns = await db_1.prisma.$queryRawUnsafe(`SELECT * FROM outreach_campaigns WHERE campaign_id = $1`, campaignId);
        if (!campaigns.length)
        ...
        const recipients = await db_1.prisma.$queryRawUnsafe(`SELECT * FROM outreach_recipients WHERE campaign_id = $1 AND status = 'pending'`, campaignId);
        if (!recipients.length)
            return { success: false, error: 'No pending recipients. Add recipients first.' };
        const todaySent = await db_1.prisma.$queryRawUnsafe(`
    ...
    `);
        ...
        await db_1.prisma.$executeRawUnsafe(`UPDATE outreach_campaigns SET status = 'sending', started_at = now(), updated_at = now() WHERE campaign_id = $1`, campaignId);
        let sentCount = 0;
        let failCount = 0;
        for (const recipient of recipients) {
            const statusCheck = await db_1.prisma.$queryRawUnsafe(`SELECT status FROM outreach_campaigns WHERE campaign_id = $1`, campaignId);
            ...
            try {
                ...
                if (result.error) {
                    await db_1.prisma.$executeRawUnsafe(`UPDATE outreach_recipients SET status = 'failed', last_error = $1 WHERE id = $2`, result.error.message || 'Unknown error', recipient.id);
                    failCount++;
                }
                else {
                ...
                await db_1.prisma.$executeRawUnsafe(`UPDATE outreach_recipients SET status = 'failed', last_error = $1 WHERE id = $2`, err.message, recipient.id);
                failCount++;
            }
        }
        const finalStatusCheck = await db_1.prisma.$queryRawUnsafe(`SELECT status FROM outreach_campaigns WHERE campaign_id = $1`, campaignId);
        ...
        await db_1.prisma.$executeRawUnsafe(`UPDATE outreach_campaigns SET status = $1, sent_count = $2, completed_at = CASE WHEN $1 = 'paused' THEN NULL ELSE now() END, updated_at = now() WHERE campaign_id = $3`, finalStatus, sentCount, campaignId);
...
async function getCampaignStats(campaignId) {
    try {
        if (campaignId) {
            ...
            if (!campaigns.length)
                return { success: false, error: 'Campaign not found' };
            const recipientStats = await db_1.prisma.$queryRawUnsafe(`
        SELECT
          status,
          COUNT(*)::int as count
...
async function getRecipientList(campaignId, status) {
    try {
        ...
        const recipients = await db_1.prisma.$queryRawUnsafe(query, ...queryParams);
...
async function pauseCampaign(campaignId) {
    try {
        await db_1.prisma.$executeRawUnsafe(`UPDATE outreach_campaigns SET status = 'paused', updated_at = now() WHERE campaign_id = $1 AND status IN ('sending', 'scheduled')`, campaignId);
...
//# sourceMappingURL=outreachCampaignService.js.map