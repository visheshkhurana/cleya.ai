...
exports.runDailySEORoutine = runDailySEORoutine;
exports.getSEOReports = getSEOReports;
exports.getSEOReportByDate = getSEOReportByDate;
exports.getLatestSEOReport = getLatestSEOReport;
 ...
 */
async function runDailySEORoutine() {
    const report = [];
    ...
    for (const url of PAGES_TO_AUDIT) {
        const result = await (0, agentTools_1.executeTool)('scout', 'analyze_seo', { url });
    ...
    const indexingResult = await (0, agentTools_1.executeTool)('scout', 'check_indexing', { urls: PAGES_TO_AUDIT });
    ...
    if (isMonday) {
        const compResult = await (0, agentTools_1.executeTool)('scout', 'analyze_competitors', { urls: COMPETITOR_URLS });
    ...
    const keywordResult = await (0, agentTools_1.executeTool)('scout', 'keyword_research', {
 ...
 */
function categorizeIssues(audits, indexingData) {
    const critical = [];
    const high = [];
    const medium = [];
 ...
 */
function analyzeGEOReadiness(audits) {
    ...
    for (const audit of audits) {
        ...
        for (const sd of audit.structuredData) {
            if (sd['@type'] === 'FAQPage' || sd['@type'] === 'Question')
                faqContent = true;
        ...
        if (/\d+%|\d+\+|\d{4,}|\d+x/i.test(desc))
            statisticsPresent = true;
        if (/is a|platform|connects|matches/i.test(desc))
            factualClaims = true;
    }
    if (!factualClaims)
        suggestions.push('Add clear factual claims AI can extract (e.g., "Cleya.ai connects 10,000+ founders...")');
    if (!faqContent)
        suggestions.push('Add FAQ structured data (FAQPage schema) for AI-search-friendly Q&A content');
    if (!statisticsPresent)
        suggestions.push('Include statistics and data points in meta descriptions and content');
    if (!entityDefinitions)
        suggestions.push('Add Organization/WebSite JSON-LD schema to clearly define Cleya.ai as an entity');
    let score = 0;
    if (factualClaims)
        score += 25;
    if (faqContent)
...
async function saveDailyReport(report) {
    ...
    await prisma.$queryRawUnsafe(`INSERT INTO seo_daily_reports (report_date, summary, full_report, pages_audited, critical_issues, high_issues, medium_issues, seo_score, geo_score, competitor_data, keyword_rankings, recommendations)
     ...
     ON CONFLICT (report_date) DO UPDATE SET
       ...
       recommendations = EXCLUDED.recommendations`, report.date, report.summary, JSON.stringify({
        auditResults: report.auditResults,
        indexingResults: report.indexingResults,
        geoAnalysis: report.geoAnalysis,
        generatedAt: new Date().toISOString(),
    }), report.pagesAudited, report.criticalIssues, report.highIssues, report.mediumIssues, report.seoScore, report.geoScore, report.competitorData ? JSON.stringify(report.competitorData) : null, JSON.stringify(report.keywordRankings), JSON.stringify(report.recommendations));
    console.log(`[SEOAutomation] Report saved for ${report.date}`);
}
/**
 * Fetch SEO daily reports from Supabase.
 */
async function getSEOReports(limit = 30) {
    return (0, supabaseClient_1.supabaseSelect)('seo_daily_reports', undefined, {
 ...
 */
async function getSEOReportByDate(date) {
    const rows = await (0, supabaseClient_1.supabaseSelect)('seo_daily_reports', { report_date: date }, { limit: 1 });
    return rows.length > 0 ? rows[0] : null;
}
/**
 * Fetch the most recent SEO report.
 */
async function getLatestSEOReport() {
    const rows = await (0, supabaseClient_1.supabaseSelect)('seo_daily_reports', undefined, {
        order: 'report_date.desc',
        limit: 1,
    });
    return rows.length > 0 ? rows[0] : null;
}
//# sourceMappingURL=seoAutomation.js.map