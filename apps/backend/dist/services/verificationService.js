...
exports.calculateVerificationScore = calculateVerificationScore;
...
async function calculateVerificationScore(userId) {
    ...
    if (factors.emailVerified)
        score += 25;
    if (factors.linkedinVerified)
        score += 25;
    if (factors.profileComplete)
        score += 15;
    if (factors.hasHeadline)
        score += 5;
    if (factors.hasCompany)
        score += 10;
    if (factors.hasIndustries)
        score += 5;
    if (factors.hasSkills)
        score += 5;
...
function getVerificationBadge(score) {
    if (score >= 75)
        return { label: 'Trusted', color: '#10B981', icon: '✓✓' };
    if (score >= 50)
        return { label: 'Verified', color: '#0D9488', icon: '✓' };
    if (score >= 25)
        return { label: 'Basic', color: '#3B82F6', icon: '○' };
