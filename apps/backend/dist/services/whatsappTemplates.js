...
const templates = {
    ...
    match_found: {
        ...
        gupshupTemplateId: undefined,
        gupshupParamOrder: undefined,
        buildMessage: (p) => {
    ...
    match_accepted: {
        ...
        buildMessage: (p) => {
            ...
            if (p.reason)
                msg += `Why I matched you: ${p.reason}\n\n`;
            if (p.linkedin)
                msg += `🔗 LinkedIn: ${p.linkedin}\n\n`;
    ...
    meeting_reminder: {
        ...
        gupshupTemplateId: 'cleya_meeting_reminder',
        gupshupParamOrder: [],
        buildMessage: (p) => `Quick reminder — your meeting starts in *${p.timeUntil || '1 hour'}*!\n\n📅 *${p.meetingTitle}*\n👤 With ${p.withName}${p.location ? `\n📍 ${p.location}` : ''}${p.meetingLink ? `\n🔗 Join: ${p.meetingLink}` : ''}`,
    },
    profile_incomplete: {
...
class WhatsAppTemplateService {
    ...
    async sendTemplate(userId, phoneNumber, templateId, params) {
        ...
        if (gupshupService_1.gupshupService.isConfigured() && template.gupshupTemplateId) {
            try {
                ...
                const result = await gupshupService_1.gupshupService.sendTemplate(userId, resolvedPhone, template.gupshupTemplateId, orderedParams);
        ...
        return messagingService_1.messagingService.sendWhatsApp(userId, resolvedPhone, message);
    }
    ...
    async triggerMatchFound(userId, matchUserId, matchScore) {
    ...
    async triggerMatchAccepted(userId, matchUserId) {
        const [user, matchUser] = await Promise.all([
            this.getUserWithPhone(userId),
            db_1.prisma.user.findUnique({
                where: { id: matchUserId },
    ...
    async triggerIntroAccepted(userId, introName) {
        const user = await this.getUserWithPhone(userId);
        if (!user)
            return null;
    ...
    async triggerMeetingScheduled(userId, meetingTitle, withName, proposedTime) {
        const user = await this.getUserWithPhone(userId);
        if (!user)
            return null;
    ...
    async triggerMeetingConfirmed(userId, meetingTitle, withName, confirmedTime, location) {
        const user = await this.getUserWithPhone(userId);
        if (!user)
            return null;
    ...
    async triggerMeetingReminder(userId, meetingTitle, withName, timeUntil, location, meetingLink) {
        const user = await this.getUserWithPhone(userId);
        if (!user)
            return null;
    ...
    async triggerProfileIncomplete(userId, completionPct) {
        const user = await this.getUserWithPhone(userId);
        if (!user)
            return null;
    ...
    async triggerEventRegistration(userId, eventName, eventDate, eventLocation) {
        const user = await this.getUserWithPhone(userId);
        if (!user)
            return null;
    ...
    async triggerEventFollowup(userId, eventName, matchList) {
        const user = await this.getUserWithPhone(userId);
        if (!user)
            return null;
    ...
    async triggerWeeklyDigest(userId, stats) {
        const user = await this.getUserWithPhone(userId);
        if (!user)
            return null;
    ...
    async triggerFollowUp(userId) {
        const user = await this.getUserWithPhone(userId);
        if (!user)
            return null;
        const pendingCount = await db_1.prisma.match.count({
            where: {
    ...
    async getUserWithPhone(userId) {
        ...
        const phone = user.whatsappPhone || user.phone || user.profile?.phoneNumber;
        if (!phone) {
...
exports.whatsappTemplates = new WhatsAppTemplateService();
