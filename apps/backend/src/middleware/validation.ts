...
const nullableArraySchema = (maxItems: number, maxItemLength: number) =>
  z.union([
    z.array(z.string().max(maxItemLength).transform(val => stripHtml(val))).max(maxItems),
    z.null().transform(() => undefined),
  ]).optional();
...
export const profileUpdateSchema = z.object({
  ...
  linkedinUrl: z.union([linkedinUrlSchema, z.null().transform(() => undefined)]).optional(),
  websiteUrl: z.union([websiteUrlSchema, z.null().transform(() => undefined)]).optional(),
  phoneNumber: z.union([phoneSchema, z.null().transform(() => undefined)]).optional(),
