...
import { verifyInboundReplyLocalPart } from '../services/matchActionToken';
 ...
 */
function detectIntent(rawText: string | undefined): 'accept' | 'decline' | 'unclear' {
  if (!rawText) return 'unclear';
  ...
  if (!head) return 'unclear';

  // Token-level pass — exact word at start gives strongest signal.
  const firstToken = head.split(/[\s.,!?]+/).filter(Boolean)[0] || '';
  if (ACCEPT_WORDS.has(firstToken)) return 'accept';
  if (DECLINE_WORDS.has(firstToken)) return 'decline';

  // Phrase pass — substring match anywhere in the head.
  for (const w of ACCEPT_WORDS) if (head.includes(w)) return 'accept';
  for (const w of DECLINE_WORDS) if (head.includes(w)) return 'decline';
...
inboundEmailRouter.get('/dns', (_req: Request, res: Response) => {
  const domain = env.REPLY_INBOUND_DOMAIN || '<your-reply-subdomain.cleya.ai>';
  res.type('text').send(
    [
      ...
      `   POST ${env.BACKEND_URL || 'https://api.cleya.ai'}/api/inbound/email`,
      '',
      `Status: ${env.REPLY_INBOUND_DOMAIN ? 'enabled' : 'disabled (REPLY_INBOUND_DOMAIN not set)'}`,
    ].join('\n'),
  );
});

inboundEmailRouter.post('/email', async (req: Request, res: Response) => {
  ...
  let matched: { matchId: string; userId: string } | null = null;
  for (const to of toAddrs) {
    const addr = (typeof to === 'string' ? to : to?.email || '').toString().toLowerCase();
    const at = addr.indexOf('@');
    if (at < 0) continue;
    const local = addr.slice(0, at);
    const verified = verifyInboundReplyLocalPart(local);
    if (verified.ok) {
  ...
  try {
    const updated = await matchingService.respondToMatch(matched.matchId, matched.userId, dbResponse);
    ...
    if (responder?.email && partner?.name) {
      emailService.sendIntroResponseAck({
        to: responder.email,
