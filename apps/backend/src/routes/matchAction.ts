...
import {
  verifyMatchActionToken,
  verifyFeedbackToken,
  FEEDBACK_REASON_LABELS,
} from '../services/matchActionToken';
 ...
 */
export const matchActionRouter = Router();
...
matchActionRouter.get('/respond', async (req: Request, res: Response) => {
  ...
  const verified = verifyMatchActionToken(tokenRaw);
  if (!verified.ok) {
    ...
    return res.status(400).type('html').send(renderConfirmation({
      ...
      message: reason,
      variant: 'error',
      cta: { label: 'Open dashboard', url: `${env.FRONTEND_URL}/matches` },
    }));
  }
  ...
  try {
    ...
    if (alreadyResponded) {
      ...
      return res.type('html').send(renderConfirmation({
        title: 'Already noted',
        message: `You've already responded to your introduction with ${partner?.name || 'this person'}. ${
          match.status === 'ACCEPTED' ? "Both of you are connected — check your inbox for the warm intro." : "We'll let you know when there's an update."
        }`,
        variant: 'info',
        cta: { label: 'Open dashboard', url: `${env.FRONTEND_URL}/matches` },
      }));
 ...
 */
matchActionRouter.get('/feedback', async (req: Request, res: Response) => {
  const tokenRaw = req.query.token;
  if (typeof tokenRaw !== 'string' || !tokenRaw) {
    return res.status(400).type('html').send(renderConfirmation({
      title: 'Link missing',
  ...
  const verified = verifyFeedbackToken(tokenRaw);
  if (!verified.ok) {
    return res.status(400).type('html').send(renderConfirmation({
      title: 'Link not valid',
  ...
  try {
    ...
    return res.type('html').send(renderConfirmation({
      ...
      message: `Got it: "${FEEDBACK_REASON_LABELS[reason]}". I'll factor this in and steer your next introductions away from the same mismatch.`,
      variant: 'success',
      cta: { label: 'Open dashboard', url: `${env.FRONTEND_URL}/matches` },
    }));
  } catch (err: any) {
