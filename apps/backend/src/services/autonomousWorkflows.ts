...
let sharedAI: ReturnType<typeof createAIService> | null = null;
function getAI() {
  if (!sharedAI) {
    if (!process.env.OPENAI_API_KEY && !process.env.ANTHROPIC_API_KEY && !process.env.GOOGLE_AI_API_KEY) return null;
    sharedAI = createAIService({ provider: 'openai', model: 'gpt-4o-mini' });
  }
  return sharedAI;
}

async function llmGenerate(agentId: string, taskType: string, prompt: string): Promise<string> {
  const ai = getAI();
  if (!ai) return '[LLM unavailable — no API key configured]';

  const routing = routeModel({ agentId, taskType });
  try {
...
export async function runNexusDailyBrief(): Promise<string> {
  console.log('[AutonomousWorkflows] Running Nexus Daily Operations Brief...');

  // Collect last runs from all agents
  const agentLogs = await safeQuery('agent logs', () =>
    supabaseSelect<any>('dm_agent_logs', {}, { order: 'created_at.desc', limit: 50 })
  ) || [];

  // Check integration health
  const integrationStatus: Record<string, boolean> = {
    ayrshare: ayrshareService.isConfigured(),
    ga4: ga4Service.isConfigured(),
    posthog: posthogService.isConfigured(),
  ...
  const ga4Metrics = await safeQuery('GA4', () => ga4Service.getMetrics('7d'));
  const posthogMetrics = await safeQuery('PostHog', () => posthogService.getMetrics('7d'));
  const sentryMetrics = await safeQuery('Sentry', () => sentryService.getMetrics('7d'));

  // Check latest Probe/Scout reports
  const latestQA = await safeQuery('QA report', () =>
    supabaseSelect<any>('qa_daily_reports', {}, { order: 'report_date.desc', limit: 1 })
  );
  const latestSEO = await safeQuery('SEO report', () =>
    supabaseSelect<any>('seo_daily_reports', {}, { order: 'report_date.desc', limit: 1 })
  );
...
Keep it concise and actionable. Focus on the Indian startup ecosystem context.`;
  ...
  // Save to Supabase
  await safeQuery('save brief', () =>
    supabaseUpsert('daily_ops_briefs', {
...
export async function runMavenContentPost(timeSlot: 'morning' | 'afternoon' | 'evening'): Promise<string> {
...
Output the post text ONLY — no JSON, no markdown fences, just the ready-to-post content.`;
  ...
  await safeQuery('save post log', () =>
    supabaseInsert('content_posts_log', {
      post_date: todayISO(),
      time_slot: timeSlot,
  ...
  await safeQuery('save to calendar', () =>
    supabaseInsert('content_calendar', {
      agent_id: 'maven',
...
export async function runLedgerFinancialMonitor(): Promise<string> {
  console.log('[AutonomousWorkflows] Running Ledger Daily Financial Monitor...');

  // Check Meta Ads campaign performance
  const metaCampaigns = await safeQuery('Meta Ads', () => adsService.getCampaigns('meta'));
  const linkedInCampaigns = await safeQuery('LinkedIn Ads', () => adsService.getCampaigns('linkedin'));
...
All amounts in INR. Keep it concise and data-driven.`;
  ...
  await safeQuery('save financial report', () =>
...
export async function runLedgerWeeklyReview(): Promise<string> {
  console.log('[AutonomousWorkflows] Running Ledger Weekly Budget Review...');

  // Get past 7 days of financial reports
  const weeklyReports = await safeQuery('weekly reports', () =>
    supabaseSelect<any>('financial_daily_reports', {}, { order: 'report_date.desc', limit: 7 })
  ) || [];
...
All amounts in INR. Be specific with numbers.`;
  ...
  await safeQuery('save weekly review', () =>
...
export async function runSentinelHealthCheck(): Promise<string> {
  ...
  for (const ep of endpointResults) {
    if (!ep.ok) criticalIssues.push(`${ep.name} is DOWN (status: ${ep.status})`);
    if (ep.responseTime > 5000) criticalIssues.push(`${ep.name} is SLOW (${ep.responseTime}ms)`);
  }
...
Focus on actionable insights. Flag anything that needs immediate attention.`;
  ...
  await safeQuery('save health report', () =>
    supabaseInsert('system_health_reports', {
...
export async function runAllyEngagementCheck(): Promise<string> {
  console.log('[AutonomousWorkflows] Running Ally Member Engagement Check...');

  // Check for new member applications
  const pendingApplications = await safeQuery('pending applications', () =>
    supabaseSelect<any>('profiles', { status: 'pending' }, { order: 'created_at.desc', limit: 20 })
  );

  // Check for recently approved members (last 24h)
  const recentApprovals = await safeQuery('recent approvals', () =>
    supabaseSelect<any>('profiles', { status: 'approved' }, { order: 'updated_at.desc', limit: 10 })
  );
  ...
  const inactiveMembers = await safeQuery('inactive members', async () => {
    ...
    return supabaseSelect<any>('profiles', {}, {
  ...
  const openTickets = await safeQuery('open tickets', () =>
    supabaseSelect<any>('tickets', { status: 'open' }, { order: 'created_at.desc', limit: 10 })
  );
...
Keep messages warm, personal, and aligned with Cleya.ai's founders community brand.`;
  ...
  await safeQuery('save engagement report', () =>
    supabaseInsert('member_engagement_reports', {
      report_date: todayISO(),
...
export async function runCatalystGrowthAnalysis(): Promise<string> {
  ...
  const recentPosts = await safeQuery('recent posts', () =>
    supabaseSelect<any>('content_posts_log', {}, { order: 'created_at.desc', limit: 10 })
  );

  // Get active experiments
  const experiments = await safeQuery('experiments', () =>
    supabaseSelect<any>('experiments', { status: 'active' }, { order: 'created_at.desc', limit: 10 })
  );
...
Think India-first distribution. Network density > raw user count.`;
  ...
  await safeQuery('save growth report', () =>
    supabaseInsert('growth_reports', {
      report_date: todayISO(),
      report_type: 'daily',
...
export async function runCatalystWeeklyReview(): Promise<string> {
  console.log('[AutonomousWorkflows] Running Catalyst Weekly Growth Review...');

  // Get past 7 days of growth reports
  const weeklyReports = await safeQuery('weekly reports', () =>
    supabaseSelect<any>('growth_reports', {}, { order: 'report_date.desc', limit: 7 })
  ) || [];
...
export async function runCloserSalesPipeline(): Promise<string> {
  console.log('[AutonomousWorkflows] Running Closer Sales Pipeline Analysis...');

  // Get pipeline data from outreach_crm
  const pipelineData = await safeQuery('pipeline', () =>
    supabaseSelect<any>('outreach_crm', {}, { order: 'score.desc', limit: 50 })
  ) || [];

  // Get membership applications (potential leads)
  const applications = await safeQuery('applications', () =>
    supabaseSelect<any>('profiles', { status: 'pending' }, { order: 'created_at.desc', limit: 20 })
  );
...
Keep messages concise, value-focused. Aim for >15% reply rate.`;
  ...
  await safeQuery('save sales report', () =>
    supabaseInsert('sales_pipeline_reports', {
      report_date: todayISO(),
      report_type: isFriday ? 'weekly' : 'daily',
      ...
      pipeline_status: {
        ...
        by_stage: pipelineData.reduce((acc: any, l: any) => {
