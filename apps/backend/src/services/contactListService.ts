...
export async function ensureContactListTables(): Promise<void> {
  try {
...
export async function createContactList(params: {
  ...
  try {
    await prisma.$executeRawUnsafe(
      `INSERT INTO contact_lists (list_id, name, description, tags) VALUES ($1, $2, $3, $4::text[])`,
      listId, params.name, params.description || '', params.tags || [],
    );
...
export async function addToContactList(params: {
  ...
  try {
    const lists: any[] = await prisma.$queryRawUnsafe(
      `SELECT list_id FROM contact_lists WHERE list_id = $1`, params.listId,
    );
    if (!lists.length) return { success: false, error: 'Contact list not found' };
    ...
    for (const c of params.contacts) {
      try {
        await prisma.$executeRawUnsafe(
          `INSERT INTO contact_list_entries (list_id, email, first_name, last_name, company, role, phone, linkedin_url, tags, custom_fields)
           ...
           ON CONFLICT (list_id, email) DO UPDATE SET
             ...
             linkedin_url = COALESCE(NULLIF(EXCLUDED.linkedin_url, ''), contact_list_entries.linkedin_url)`,
          params.listId,
    ...
    await prisma.$executeRawUnsafe(
      `UPDATE contact_lists SET contact_count = (SELECT COUNT(*) FROM contact_list_entries WHERE list_id = $1), updated_at = now() WHERE list_id = $1`,
      params.listId,
    );
...
export async function getContactLists(tag?: string): Promise<any> {
  try {
    ...
    const lists: any[] = await prisma.$queryRawUnsafe(query, ...queryParams);
...
export async function getListContacts(params: {
  listId: string;
  limit?: number;
  offset?: number;
}): Promise<any> {
  ...
  try {
    const contacts: any[] = await prisma.$queryRawUnsafe(
      `SELECT id, email, first_name, last_name, company, role, phone, linkedin_url, tags, custom_fields, source, created_at
       FROM contact_list_entries WHERE list_id = $1 ORDER BY created_at DESC LIMIT $2 OFFSET $3`,
      params.listId, limit, offset,
    );
...
export async function importListToCampaign(params: {
  listId: string;
  campaignId: string;
}): Promise<any> {
  try {
    const contacts: any[] = await prisma.$queryRawUnsafe(
      `SELECT email, first_name, last_name, company, role FROM contact_list_entries WHERE list_id = $1`,
      params.listId,
    );
    if (!contacts.length) return { success: false, error: 'Contact list is empty' };
    ...
    for (const c of contacts) {
      ...
      try {
        await prisma.$executeRawUnsafe(
          `INSERT INTO outreach_recipients (campaign_id, email, first_name, last_name, company, role)
           VALUES ($1, $2, $3, $4, $5, $6)
    ...
    await prisma.$executeRawUnsafe(
      `UPDATE outreach_campaigns SET total_recipients = (SELECT COUNT(*) FROM outreach_recipients WHERE campaign_id = $1), updated_at = now() WHERE campaign_id = $1`,
      params.campaignId,
    );
...
export async function deleteContactList(listId: string): Promise<any> {
  try {
    await prisma.$executeRawUnsafe(`DELETE FROM contact_list_entries WHERE list_id = $1`, listId);
    await prisma.$executeRawUnsafe(`DELETE FROM contact_lists WHERE list_id = $1`, listId);
