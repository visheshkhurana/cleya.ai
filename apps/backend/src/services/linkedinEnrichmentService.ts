...
class LinkedinEnrichmentService {
  async enrichProfile(userId: string): Promise<boolean> {
    try {
      ...
      if (!profile || !profile.linkedinUrl) return false;

      const existingExtra = (profile.extraData as Record<string, any>) || {};
      ...
      await prisma.profile.update({
        where: { userId },
        data: { extraData: updatedExtra as any },
      });

      try {
        const fullProfile = await prisma.profile.findUnique({ where: { userId } });
        if (fullProfile) {
          await generateAndStoreEmbedding(userId, fullProfile as any);
  ...
  private async extractSignals(profile: {
...
If information is not available for a field, use an empty array. Be conservative — only include what can be reasonably inferred from the provided data. Do not fabricate specific details.`,
  ...
  async enrichBatch(batchSize = 10): Promise<{ total: number; enriched: number; skipped: number; errors: number }> {
    ...
    const needsEnrichment = profiles.filter(p => {
      const extra = p.extraData as Record<string, any> | null;
      if (!extra?.enrichedData?.enrichedAt) return true;
  ...
  async onNewUserSignup(userId: string): Promise<void> {
    setTimeout(async () => {
      try {
