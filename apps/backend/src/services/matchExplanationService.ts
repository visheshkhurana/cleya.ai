...
const FACTOR_DEFS: Array<{ key: string; label: string; weight: number }> = [
...
export class MatchExplanationService {
  async getForUser(matchId: string, userId: string): Promise<MatchExplanation | null> {
    ...
    const breakdown: any = match.scoreBreakdown || {};
    const factors: ExplanationFactor[] = FACTOR_DEFS.map((f) => ({
      key: f.key,
      label: f.label,
    ...
    const myAccepted = await prisma.match.findMany({
      where: {
        status: 'ACCEPTED',
        OR: [{ userAId: userId }, { userBId: userId }],
      },
    ...
    const otherAccepted = await prisma.match.findMany({
      where: {
        status: 'ACCEPTED',
    ...
    for (const m of otherAccepted) {
      ...
      if (mySet.has(counterpart)) mutual++;
    }

    // Recent activity (last match update)
    const lastActivity = await prisma.match.findFirst({
  ...
  private buildSummary(factors: ExplanationFactor[], mutual: number): string {
    ...
    if (mutual > 0) s += ` You share ${mutual} mutual connection${mutual === 1 ? '' : 's'}.`;
    return s;
  }

  private formatActivity(d: Date): string {
...
export const matchExplanationService = new MatchExplanationService();
