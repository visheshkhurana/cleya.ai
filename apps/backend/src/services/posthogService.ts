...
async function getMetrics(dateRange: '7d' | '30d' | '90d' = '30d'): Promise<PostHogMetrics | null> {
  ...
  try {
    ...
    try {
      const totalData = await phFetch('/api/projects/@current/insights/trend/', {
    ...
    try {
      const uniqueData = await phFetch('/api/projects/@current/insights/trend/', {
    ...
    try {
      const eventBreakdown = await phFetch('/api/projects/@current/insights/trend/', {
        events: [
    ...
    try {
      const sessionData = await phFetch('/api/projects/@current/insights/trend/', {
    ...
    try {
      const retentionData = await phFetch('/api/projects/@current/insights/retention/', {
    ...
    try {
      const featureData = await phFetch('/api/projects/@current/insights/trend/', {
        events: [
    ...
    console.error('PostHog fetch error:', message);
    return null;
  }
}
