...
export default function AdminDashboard() {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#0F172A]">
        <div className="flex items-center gap-3">
          ...
          <p className="text-teal-300">Loading dashboard...</p>
        </div>
      </div>
    );
  }
  ...
  const statusColor = (status: string) => {
    ...
    return map[status] || 'text-teal-300 bg-teal-500/10 border-teal-500/20';
  };

  return (
    <div className="min-h-screen bg-[#0F172A]">
      <header className="bg-[#1E293B]/80 backdrop-blur-sm border-b border-teal-500/10 px-4 sm:px-6 py-3 sm:py-4 sticky top-0 z-10">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-teal-500 to-teal-600 text-white text-sm font-bold flex items-center justify-center shadow-lg shadow-teal-500/20">
              C
            </div>
          ...
          <button
            ...
            className="text-xs sm:text-sm text-slate-400 hover:text-teal-200 transition"
          >
            Sign out
          </button>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 sm:py-6">
        <div className="flex gap-1 mb-6 p-1 rounded-xl bg-teal-900/20 border border-teal-500/10 overflow-x-auto">
          {[
        ...
        {activeTab === 'overview' && (
          <div className="space-y-8">
            {stats && (
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {[
                  ...
                  <div
                    key={stat.label}
                    className="bg-[#1E293B]/60 backdrop-blur-sm rounded-xl border border-teal-500/10 p-5 hover:border-teal-500/20 transition"
                  >
            ...
            <div className="bg-[#1E293B]/60 backdrop-blur-sm rounded-xl border border-teal-500/10 p-6">
              <h2 className="text-sm font-semibold text-white mb-6">Conversion Funnel</h2>
              <div className="space-y-3">
                {funnel.map((step, i) => (
                  <div key={step.stage} className="flex items-center gap-4">
                    <div className="w-44 text-xs font-medium text-teal-300/70 text-right">{step.stage}</div>
                    <div className="flex-1 bg-teal-900/20 rounded-full h-8 overflow-hidden">
                      <div
                        className="h-full bg-gradient-to-r from-teal-600 to-teal-500 rounded-full flex items-center justify-end pr-3 transition-all duration-700"
                    ...
                    {i > 0 && funnel[i - 1].count > 0 && (
                      <div className="w-16 text-xs text-teal-400/50">
            ...
            <div className="bg-[#1E293B]/60 backdrop-blur-sm rounded-xl border border-teal-500/10 overflow-hidden">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-teal-500/5">
                    {users.map((user) => (
                      <tr key={user.id} className="hover:bg-teal-900/10 transition">
                        <td className="px-6 py-3 font-medium text-white">{user.email}</td>
                        <td className="px-6 py-3">
                          {user.profile?.persona ? (
                            <span className="px-2.5 py-1 rounded-full text-xs bg-teal-500/15 text-teal-300 border border-teal-500/20">
                              {user.profile.persona}
                            </span>
                          ) : (
                        ...
                        <td className="px-6 py-3 text-teal-200/70">{user.profile?.companyName || '-'}</td>
                        <td className="px-6 py-3">
                          <div className="flex items-center gap-2">
                            <div className="w-20 bg-teal-900/20 rounded-full h-1.5">
                              <div
                                className="bg-gradient-to-r from-teal-500 to-teal-400 rounded-full h-1.5 transition-all"
                        ...
                        <td className="px-6 py-3">
                          <div className="flex gap-2">
                            <button
                              ...
                              className="px-2 py-1 text-xs rounded-md bg-teal-600/20 text-teal-300 hover:bg-teal-600/40 transition border border-teal-500/20"
                            >
                              Call
                            </button>
                            <button
            ...
            {commData && (
              <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                {[
                  ...
                  <div
                    key={stat.label}
                    className="bg-[#1E293B]/60 backdrop-blur-sm rounded-xl border border-teal-500/10 p-5"
                  >
                    <div className="flex items-center gap-2 mb-2">
                      <span className="text-lg">{stat.icon}</span>
                      <p className="text-xs font-medium text-slate-400 uppercase tracking-wider">{stat.label}</p>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-teal-500/5">
                    {(commData?.calls || []).map((call: any) => (
                      <tr key={call.id} className="hover:bg-teal-900/10 transition">
                        <td className="px-6 py-3 text-white">{call.user?.email || '-'}</td>
                        <td className="px-6 py-3 text-teal-200/70">{call.phoneNumber}</td>
                        <td className="px-6 py-3">
                          <span className={`text-xs ${call.direction === 'INBOUND' ? 'text-blue-400' : 'text-teal-300'}`}>
                        ...
                        <td className="px-6 py-3 text-teal-200/70">
                          {call.duration ? `${Math.floor(call.duration / 60)}m ${call.duration % 60}s` : '-'}
                        </td>
                        <td className="px-6 py-3 text-teal-300/40 text-xs">{formatDate(call.createdAt)}</td>
                      </tr>
                    ))}
                    {(!commData?.calls || commData.calls.length === 0) && (
                      <tr>
                        <td colSpan={6} className="px-6 py-8 text-center text-teal-400/40">No call records yet</td>
                      </tr>
                    )}
                  </tbody>
                </table>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-teal-500/5">
                    {(commData?.messages || []).map((msg: any) => (
                      <tr key={msg.id} className="hover:bg-teal-900/10 transition">
                        <td className="px-6 py-3 text-white">{msg.user?.email || '-'}</td>
                        <td className="px-6 py-3 text-teal-200/70">{msg.recipientPhone}</td>
                        <td className="px-6 py-3">
                        ...
                        <td className="px-6 py-3 text-teal-200/70 max-w-xs truncate">{msg.content}</td>
                        <td className="px-6 py-3">
                          <span className={`px-2 py-0.5 rounded-full text-xs border ${statusColor(msg.status)}`}>
                            {msg.status}
                          </span>
                        </td>
                        <td className="px-6 py-3 text-teal-300/40 text-xs">{formatDate(msg.createdAt)}</td>
                      </tr>
                    ))}
                    {(!commData?.messages || commData.messages.length === 0) && (
                      <tr>
                        <td colSpan={6} className="px-6 py-8 text-center text-teal-400/40">No message records yet</td>
                      </tr>
                    )}
                  </tbody>
                </table>
            ...
            {dealData && (
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {[
                  ...
                  <div key={stat.label} className="bg-[#1E293B]/60 backdrop-blur-sm rounded-xl border border-teal-500/10 p-5">
                    <div className="flex items-center gap-2 mb-2">
                      <span className="text-lg">{stat.icon}</span>
                      <p className="text-xs font-medium text-slate-400 uppercase tracking-wider">{stat.label}</p>
                    </div>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-teal-500/5">
                    {(dealData?.deals || []).map((deal: any) => (
                      <tr key={deal.id} className="hover:bg-teal-900/10 transition">
                        <td className="px-6 py-3 text-white">{deal.dealPartner?.email || '-'}</td>
                        <td className="px-6 py-3">
                          <div>
                            <div className="text-white">{deal.founder?.email || '-'}</div>
                            <div className="text-xs text-teal-300/50">{deal.founder?.profile?.companyName || ''}</div>
                          </div>
                        </td>
                        <td className="px-6 py-3 text-teal-200/70">{deal.industry || '-'}</td>
                        <td className="px-6 py-3 text-teal-200/70">{deal.stage?.replace(/_/g, ' ') || '-'}</td>
                        <td className="px-6 py-3">
                          <select
                            value={deal.status}
                            onChange={async (e) => {
                            ...
                            className="bg-[#0F172A] border border-teal-500/20 rounded px-2 py-1 text-xs text-teal-200"
                          >
                        ...
                        <td className="px-6 py-3">
                          {deal.introSent ? (
                            <div>
                              ...
                              {deal.introDate && <div className="text-teal-300/40 text-xs">{formatDate(deal.introDate)}</div>}
                            </div>
                          ) : (
                        ...
                        <td className="px-6 py-3">
                          <button
                            ...
                            className="px-2 py-1 text-xs bg-teal-600/20 text-teal-300 rounded hover:bg-teal-600/40 transition"
                          >
                            Set Value
                          </button>
                        </td>
                    ...
                    {(!dealData?.deals || dealData.deals.length === 0) && (
                      <tr>
                        <td colSpan={9} className="px-6 py-8 text-center text-teal-400/40">No deals tracked yet</td>
                      </tr>
                    )}
                  </tbody>
                </table>
            ...
            <div className="bg-[#1E293B]/60 backdrop-blur-sm rounded-xl border border-teal-500/10 overflow-hidden">
              <div className="px-6 py-4 border-b border-teal-500/10 flex items-center justify-between">
                ...
                <button
                  ...
                  className="px-3 py-1.5 text-xs rounded-lg bg-teal-600 text-white hover:bg-teal-500 transition shadow-lg shadow-teal-600/20"
                >
                  + New Event
                </button>
              </div>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-teal-500/5">
                    {(eventData?.events || []).map((event: any) => (
                      <tr key={event.id} className="hover:bg-teal-900/10 transition">
                        <td className="px-6 py-3">
                          <div>
                            <div className="text-white font-medium">{event.name}</div>
                            {event.description && <div className="text-xs text-teal-300/50 mt-0.5 max-w-xs truncate">{event.description}</div>}
                          </div>
                        </td>
                        <td className="px-6 py-3 text-teal-200/70 text-xs">{formatDate(event.date)}</td>
                        <td className="px-6 py-3 text-teal-200/70">
                          {event.isVirtual ? (
                        ...
                        <td className="px-6 py-3">
                          <span className={`px-2 py-0.5 rounded-full text-xs border ${
                            ...
                            event.status === 'COMPLETED' ? 'text-teal-300 bg-teal-500/10 border-teal-500/20' :
                        ...
                        <td className="px-6 py-3 text-teal-200/70">{event.maxCapacity || 'Unlimited'}</td>
                      </tr>
                    ))}
                    {(!eventData?.events || eventData.events.length === 0) && (
                      <tr>
                        <td colSpan={6} className="px-6 py-8 text-center text-teal-400/40">No events yet</td>
                      </tr>
                    )}
                  </tbody>
                </table>
      ...
      {showCreateEvent && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="bg-[#1E293B] border border-teal-500/20 rounded-2xl p-6 w-full max-w-md shadow-2xl">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2.5 rounded-xl border border-teal-500/20 bg-teal-900/20 text-white placeholder-slate-400/30 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500"
                />
              </div>
              <div>
                ...
                <textarea
                  ...
                  rows={2}
                  className="w-full px-3 py-2.5 rounded-xl border border-teal-500/20 bg-teal-900/20 text-white placeholder-slate-400/30 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500 resize-none"
                />
              </div>
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2.5 rounded-xl border border-teal-500/20 bg-teal-900/20 text-white text-sm focus:outline-none focus:ring-2 focus:ring-teal-500"
                />
              </div>
              <div>
              ...
              <div className="flex items-center gap-3">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    ...
                    className="rounded border-teal-500/20 bg-teal-900/20 text-teal-600"
                  />
                  <span className="text-sm text-teal-300">Virtual event</span>
                </label>
              </div>
              <div>
            ...
            <div className="flex justify-end gap-3 mt-6">
              <button
                ...
                className="px-4 py-2 text-sm text-slate-400 hover:text-teal-200 transition"
              >
                Cancel
              </button>
              <button
                ...
                className="px-4 py-2 text-sm bg-teal-600 text-white rounded-xl hover:bg-teal-500 transition disabled:opacity-40 disabled:cursor-not-allowed shadow-lg shadow-teal-600/20"
              >
                Create Event
              </button>
            </div>
      ...
      {activeTab === 'analytics' && (
        <div className="space-y-6">
          {!analyticsData ? (
            <div className="text-center py-12 text-teal-300/40">Loading analytics...</div>
          ) : (
            <>
              <div className="flex items-center justify-between">
                ...
                <button onClick={handleSendDigest} disabled={digestLoading}
                  className="px-4 py-2 rounded-xl text-sm font-medium bg-teal-600 text-white hover:bg-teal-500 transition disabled:opacity-40">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {[
                  ...
                  <div key={i} className="rounded-xl border border-teal-500/10 p-4" style={{ background: 'rgba(30,41,59,0.6)' }}>
                    <div className="flex items-center gap-2 mb-2">
                      <span className="text-lg">{s.icon}</span>
                      <span className="text-xs text-teal-300/40 uppercase font-medium">{s.label}</span>
                    </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="rounded-xl border border-teal-500/10 p-5" style={{ background: 'rgba(30,41,59,0.6)' }}>
                    ...
                    {analyticsData.dailySignups?.map((d: any, i: number) => {
                      ...
                      return (
                        <div key={i} className="flex-1 flex flex-col items-center gap-1">
                          <span className="text-xs text-teal-300/40">{d.count}</span>
                          <div className="w-full rounded-t-md" style={{
                            height: `${Math.max(height, 4)}%`,
                            background: 'linear-gradient(180deg, #0D9488, #0F766E)',
                            minHeight: '4px',
                          }} />
                          <span className="text-[10px] text-teal-300/30">{d.date.slice(5)}</span>
                        </div>
                      );
                    })}
                  </div>
                ...
                <div className="rounded-xl border border-teal-500/10 p-5" style={{ background: 'rgba(30,41,59,0.6)' }}>
                  ...
                  <div className="space-y-2">
                      ...
                      return (
                        <div key={i}>
                          <div className="flex justify-between text-xs mb-1">
                            <span className="text-teal-200/60">{p.persona}</span>
                            <span className="text-teal-300/40">{p.count} ({pct}%)</span>
                          </div>
                          <div className="h-2 rounded-full bg-teal-900/30">
                    ...
                    {(!analyticsData.personaBreakdown || analyticsData.personaBreakdown.length === 0) && (
                      <p className="text-teal-300/30 text-xs">No persona data yet</p>
                    )}
                  </div>
                </div>
              </div>
                ...
                <div className="space-y-2">
                  {analyticsData.attributionBreakdown?.map((a: any, i: number) => {
                    ...
                    return (
                      <div key={i}>
                        <div className="flex justify-between text-xs mb-1">
                          <span className="text-teal-200/60">{sourceLabels[a.source] || a.source}</span>
                          <span className="text-teal-300/40">{a.count} ({pct}%)</span>
                        </div>
                        <div className="h-2 rounded-full bg-teal-900/30">
                          <div className="h-full rounded-full" style={{
                            width: `${pct}%`,
                  ...
                  {(!analyticsData.attributionBreakdown || analyticsData.attributionBreakdown.length === 0) && (
                    <p className="text-teal-300/30 text-xs">No attribution data yet</p>
                  )}
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="rounded-xl border border-teal-500/10 p-5" style={{ background: 'rgba(30,41,59,0.6)' }}>
                  ...
                  <div className="flex items-center gap-4 mb-4">
                    <div className="text-center">
                      ...
                      <p className="text-xs text-teal-300/40">Avg Rating</p>
                    </div>
                    <div className="text-center">
                      <p className="text-3xl font-bold text-teal-400">{analyticsData.feedbackStats?.total || 0}</p>
                      <p className="text-xs text-teal-300/40">Total Reviews</p>
                    </div>
                  </div>
                    ...
                    {[5, 4, 3, 2, 1].map(r => {
                      ...
                      return (
                        <div key={r} className="flex items-center gap-2">
                          ...
                          <div className="flex-1 h-2 rounded-full bg-teal-900/30">
                            <div className="h-full rounded-full bg-yellow-400/60" style={{ width: `${(count / total) * 100}%` }} />
                          </div>
                          <span className="text-xs text-teal-300/40 w-6 text-right">{count}</span>
                        </div>
                      );
                    })}
                  </div>
                ...
                <div className="rounded-xl border border-teal-500/10 p-5" style={{ background: 'rgba(30,41,59,0.6)' }}>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="rounded-lg border border-teal-500/10 p-3 text-center" style={{ background: 'rgba(13,148,136,0.05)' }}>
                      <p className="text-xl font-bold text-teal-400">{analyticsData.totalCalls}</p>
                      <p className="text-xs text-teal-300/40">Calls</p>
                    </div>
                    <div className="rounded-lg border border-teal-500/10 p-3 text-center" style={{ background: 'rgba(13,148,136,0.05)' }}>
                      <p className="text-xl font-bold text-blue-400">{analyticsData.totalMessages}</p>
                      <p className="text-xs text-teal-300/40">Messages</p>
                    </div>
                  </div>
                  <div className="mt-3 space-y-1.5">
                    {analyticsData.channelBreakdown?.map((c: any, i: number) => (
                      <div key={i} className="flex justify-between text-xs">
                        <span className="text-teal-200/60">{c.channel}</span>
                        <span className="text-teal-300/40">{c.count}</span>
                      </div>
                    ))}
                  </div>
                </div>
                  ...
                  {analyticsData.recentActivity?.map((a: any, i: number) => (
                    <div key={i} className="flex items-center gap-3 px-3 py-2 rounded-lg" style={{ background: 'rgba(13,148,136,0.03)' }}>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {a.email && <span className="text-[10px] text-teal-300/30">{a.email}</span>}
                      </div>
                      <span className="text-[10px] text-teal-300/20 flex-shrink-0">
                        {new Date(a.createdAt).toLocaleDateString()}
                      </span>
                    </div>
                  ))}
                  {(!analyticsData.recentActivity || analyticsData.recentActivity.length === 0) && (
                    <p className="text-teal-300/30 text-xs text-center py-4">No recent activity</p>
                  )}
                </div>
              </div>
            </>
          ...
          <div className="bg-[#1E293B] border border-teal-500/20 rounded-2xl p-6 w-full max-w-md shadow-2xl">
                        ...
                        <button
                          ...
                          className={`px-4 py-2 rounded-lg text-sm font-medium border transition ${
                            triggerChannel === ch
                              ...
                              : 'border-teal-500/20 text-slate-400 hover:text-teal-200'
                          }`}
                        >
            ...
            <div className="flex gap-3 mt-6">
              <button
                ...
                className="flex-1 px-4 py-2.5 rounded-xl text-sm font-medium text-slate-400 border border-teal-500/20 hover:text-teal-200 transition"
              >
                Cancel
              </button>
              <button
                ...
                className="flex-1 px-4 py-2.5 rounded-xl text-sm font-medium bg-teal-600 text-white hover:bg-teal-500 transition disabled:opacity-40"
              >
