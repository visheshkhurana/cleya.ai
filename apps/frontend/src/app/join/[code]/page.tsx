...
export default function JoinPage() {
  ...
  if (!inviteInfo?.valid) {
    return (
      <AppShell>
        <div className="max-w-md w-full text-center rounded-2xl border border-white/[0.08] p-8" style={{ background: 'rgba(15,22,41,0.8)' }}>
    ...
    <AppShell>
      <div className="w-full max-w-sm">
        <div className="text-center mb-6">
          ...
          <h1 className="font-sans text-2xl font-bold text-white mb-1">You&apos;re invited to Cleya</h1>
          {inviteInfo.inviterName && (
            <p className="text-sm" style={{ color: '#94A3B8' }}>
              <span style={{ color: '#9B95FF' }}>{inviteInfo.inviterName}</span>
              {inviteInfo.inviterTitle ? ` (${inviteInfo.inviterTitle})` : ''} thinks you should join India&apos;s AI networking community.
            </p>
          )}
        </div>

        <div className="rounded-2xl border border-white/[0.08] p-8" style={{ background: 'rgba(15,22,41,0.8)', backdropFilter: 'blur(20px)' }}>
          <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-3 gap-2">
                {[
                  ...
                  <button key={p.value} type="button" onClick={() => setSelectedPersona(p.value)}
                    ...
                    style={{
                      background: selectedPersona === p.value ? 'rgba(108,99,255,0.15)' : 'rgba(255,255,255,0.03)',
                      borderColor: selectedPersona === p.value ? '#6C63FF' : 'rgba(255,255,255,0.08)',
                    }}>
                    <span className="text-lg">{p.icon}</span>
                    <span className="text-xs font-medium" style={{ color: selectedPersona === p.value ? '#9B95FF' : '#94A3B8' }}>{p.label}</span>
                  </button>
                ))}
              </div>
            </div>
            ...
            <label className="flex items-start gap-2 cursor-pointer">
              <input type="checkbox" checked={consent} onChange={e => setConsent(e.target.checked)} className="mt-0.5 rounded border-white/20 bg-white/5 accent-brand-violet" />
              <span className="text-xs leading-relaxed" style={{ color: '#94A3B8' }}>
                I agree to the <a href="/terms" target="_blank" className="underline" style={{ color: '#9B95FF' }}>Terms of Service</a> and <a href="/privacy" target="_blank" className="underline" style={{ color: '#9B95FF' }}>Privacy Policy</a>
              </span>
            </label>
            {error && (
          ...
          <p className="text-center text-xs mt-4" style={{ color: '#94A3B8' }}>
            Already have an account? <a href="/?action=login" className="font-medium" style={{ color: '#9B95FF' }}>Log in</a>
          </p>
        </div>
      </div>
    </AppShell>
