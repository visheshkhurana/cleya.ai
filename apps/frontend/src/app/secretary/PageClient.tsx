...
export default function SecretaryPage() {
  ...
  return (
    <div className="min-h-screen bg-[#0F172A] flex flex-col">
      <nav className="sticky top-0 z-50 border-b border-white/[0.06] bg-[#0F172A]/95 backdrop-blur-xl">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <a href="/dashboard" className="flex items-center gap-2">
              <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-[#0D9488] to-[#0F766E] flex items-center justify-center text-white font-bold text-sm">C</div>
      ...
      <div className="flex-1 flex flex-col max-w-4xl mx-auto w-full">
        <div className="px-4 py-3 flex items-center justify-between border-b border-white/[0.06]">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-gradient-to-br from-[#0D9488] to-[#0F766E] flex items-center justify-center">
        ...
        <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 py-6 space-y-4" style={{ maxHeight: 'calc(100vh - 200px)' }}>
          {messages.length === 0 && historyLoaded && (
            <div className="flex flex-col items-center justify-center py-16 text-center">
              <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-[#0D9488]/20 to-[#0F766E]/20 border border-[#0D9488]/20 flex items-center justify-center mb-6">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 max-w-lg">
                {quickPrompts.map((p, i) => (
                  <button key={i} onClick={() => sendMessage(p.label)}
                    className="px-3 py-2.5 rounded-xl bg-white/[0.04] border border-white/[0.06] text-white/60 text-xs hover:bg-white/[0.08] hover:text-white hover:border-[#0D9488]/30 transition-all text-left">
            ...
            <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[80%] rounded-2xl px-4 py-3 ${
                msg.role === 'user'
                  ...
                  : 'bg-[#1E293B] text-white/80 border border-white/[0.06]'
              }`}>
                  ...
                  <div className="mt-3 space-y-2 border-t border-white/10 pt-3">
                    ...
                    {msg.actions.map((action: any, j: number) => (
                      <button key={j} onClick={() => executeAction(action, i * 100 + j)}
                        ...
                        className="w-full text-left px-3 py-2 rounded-lg bg-[#0D9488]/10 border border-[#0D9488]/20 text-[#5eead4] text-xs hover:bg-[#0D9488]/20 transition-all disabled:opacity-40">
          ...
          {loading && (
            <div className="flex justify-start">
              <div className="bg-[#1E293B] border border-white/[0.06] rounded-2xl px-4 py-3">
        ...
        <div className="border-t border-white/[0.06] px-4 py-3">
          <form onSubmit={(e) => { e.preventDefault(); sendMessage(); }} className="flex items-center gap-2">
            <input
              ...
              className="flex-1 px-4 py-3 rounded-xl bg-[#1E293B] border border-white/[0.08] text-white placeholder-white/30 text-sm focus:outline-none focus:ring-2 focus:ring-[#0D9488]/50 focus:border-transparent"
              disabled={loading}
            />
            <button type="submit" disabled={!input.trim() || loading}
              className="w-11 h-11 rounded-xl bg-gradient-to-r from-[#0D9488] to-[#0F766E] text-white flex items-center justify-center hover:opacity-90 transition-opacity disabled:opacity-30">
