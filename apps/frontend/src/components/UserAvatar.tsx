...
export default function UserAvatar({ name, avatarUrl, size = 'md', className = '' }: UserAvatarProps) {
  ...
  return (
    <div
      className={`${container} rounded-full flex items-center justify-center ${text} font-bold flex-shrink-0 bg-gradient-to-br from-teal-500 to-teal-700 text-white ${className}`}
    >
      {getInitials(name)}
    </div>
  );
