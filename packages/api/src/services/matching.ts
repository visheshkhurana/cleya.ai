...
const TEST_DOMAIN_SQL_PREDICATE = TEST_EMAIL_DOMAINS
...
async function getRuleBasedCandidates(
  ...
  return profiles
    .filter((p: any) => p.user?.name && String(p.user.name).trim().length > 0)
    .map((p: any) => mapProfileToMatching(p));
}
