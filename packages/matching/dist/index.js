...
const SECTOR_FAMILY_SIMILARITY = {
...
function normalizeToExpertiseTags(inputs) {
    const tags = new Set();
    for (const input of inputs) {
        const lower = input.toLowerCase().trim();
        if (!lower || lower.length < 2)
            continue;
        for (const [tag, keywords] of Object.entries(exports.EXPERTISE_TAGS)) {
...
class MatchingEngine {
    ...
    intentWeight = 0.35;
    semanticWeight = 0.30;
    sectorFamilyCache = new Map();
    getSectorFamily(industry) {
        const key = industry.toLowerCase().trim();
        if (this.sectorFamilyCache.has(key))
            return this.sectorFamilyCache.get(key);
    ...
    scoreSectorSimilarity(industriesA, industriesB) {
        if (!industriesA.length || !industriesB.length)
            return 0.3;
    ...
    computeCompatibilitySignals(profileA, profileB) {
        const industryMatch = this.scoreSectorSimilarity(profileA.industries, profileB.industries);
        ...
        const addTraction = (p) => {
            if (p.tractionMetrics?.revenue)
                tractionHighlights.push(`Revenue: ₹${(p.tractionMetrics.revenue / 100000).toFixed(1)}L`);
            if (p.tractionMetrics?.mrr)
                tractionHighlights.push(`MRR: ₹${(p.tractionMetrics.mrr / 100000).toFixed(1)}L`);
            if (p.tractionMetrics?.userCount)
                tractionHighlights.push(`Users: ${p.tractionMetrics.userCount.toLocaleString()}`);
            if (p.tractionMetrics?.growthRate)
                tractionHighlights.push(`Growth: ${p.tractionMetrics.growthRate}% MoM`);
            if (p.keyTractionPoints)
                tractionHighlights.push(p.keyTractionPoints);
        };
        ...
        if (conflict > 0)
            conflictFlags.push('Portfolio sector conflict detected');
        if (profileA.openToMeeting === false)
            conflictFlags.push(`${profileA.persona} not open to meeting`);
        if (profileB.openToMeeting === false)
            conflictFlags.push(`${profileB.persona} not open to meeting`);
        return {
    ...
    passesIntentFilter(user, candidate) {
        const userIntents = user.lookingFor;
        const neededExpertise = user.matchingExpertiseNeeded || [];
        if (!userIntents.length && !user.priority && !user.matchingGoal && !neededExpertise.length)
            return true;
        for (const intent of userIntents) {
            const idealPersonas = INTENT_TO_PERSONA[intent];
            if (idealPersonas && idealPersonas.includes(candidate.persona))
                return true;
        }
        if (user.priority) {
            const boostPersonas = PRIORITY_PERSONA_BOOST[user.priority];
            if (boostPersonas && boostPersonas.includes(candidate.persona))
                return true;
        }
        ...
        if (user.matchingGoal) {
            ...
            if (goalPersonas && goalPersonas.includes(candidate.persona))
                return true;
        }
        return false;
    }
    ...
    scoreFounderContextMatch(a, b) {
        const scoreOneDirection = (founder, candidate) => {
            if (founder.persona === 'FOUNDER' && founder.priority === 'FUNDRAISING') {
                if (!['INVESTOR', 'DEAL_PARTNER', 'VENTURE_PARTNER'].includes(candidate.persona))
                    return 0;
                const industryOverlap = this.scoreSectorSimilarity(founder.industries, candidate.industries);
                if (industryOverlap < 0.20 && founder.industries.length > 0 && candidate.industries.length > 0)
                    return 0.1;
                let score = 0.4;
                ...
                if (founder.companyStage && candidate.companyStage) {
                    ...
                    if (compatible.includes(candidate.companyStage))
                        score += 0.25;
                }
                else {
                    score += 0.1;
            ...
            if (founder.persona === 'FOUNDER' && founder.priority === 'COFOUNDER') {
                if (!['TALENT', 'FOUNDER'].includes(candidate.persona))
                    return 0;
                let score = 0.3;
            ...
            if (founder.persona === 'FOUNDER' && founder.priority === 'HIRING') {
                if (!['TALENT', 'JOB_SEEKER'].includes(candidate.persona))
                    return 0;
                let score = 0.3;
                if (candidate.targetRole) {
                    const roleToFounderNeed = {
    ...
    scoreTractionStageAlignment(a, b) {
        const scoreOneDir = (investor, founder) => {
            if (!['INVESTOR', 'VENTURE_PARTNER', 'DEAL_PARTNER'].includes(investor.persona))
                return 0.5;
            if (founder.persona !== 'FOUNDER')
                return 0.5;
            let score = 0.3;
            ...
            if (stage === 'PRE_SEED' || stage === 'SEED') {
                if (traction.mrr && traction.mrr > 0)
                    score += 0.3;
                if (traction.userCount && traction.userCount > 100)
                    score += 0.2;
            }
            else if (stage === 'SERIES_A') {
                if (traction.mrr && traction.mrr >= 500000)
                    score += 0.3;
                else if (traction.revenue && traction.revenue >= 5000000)
                    score += 0.3;
                if (traction.growthRate && traction.growthRate >= 15)
                    score += 0.2;
            }
            else if (stage === 'SERIES_B' || stage === 'SERIES_C_PLUS') {
                if (traction.revenue && traction.revenue >= 50000000)
                    score += 0.3;
                if (traction.growthRate && traction.growthRate >= 20)
                    score += 0.2;
            }
            if (traction.runway && traction.runway <= 6)
                score += 0.1;
            return Math.min(score, 1.0);
        };
    ...
    detectPortfolioConflict(a, b) {
        const checkConflict = (investor, founder) => {
            if (!['INVESTOR', 'VENTURE_PARTNER'].includes(investor.persona))
                return 0;
            if (founder.persona !== 'FOUNDER')
                return 0;
            if (!investor.portfolioCompanies?.length || !founder.industries.length)
                return 0;
            const founderSectors = new Set(founder.industries.map(s => s.toLowerCase().trim()));
            const founderFamilies = new Set();
            for (const s of founderSectors) {
                const fam = this.getSectorFamily(s);
                if (fam)
                    founderFamilies.add(fam);
            }
            let directMatches = 0;
            let familyMatches = 0;
            ...
            if (directMatches === 0 && familyMatches === 0)
                return 0;
    ...
    scoreTalentPreferences(a, b) {
        const scoreOneDir = (talent, employer) => {
            if (!['TALENT', 'JOB_SEEKER'].includes(talent.persona))
                return 0.5;
            if (!['FOUNDER', 'OPERATOR'].includes(employer.persona))
                return 0.5;
            let score = 0.5;
            let checks = 0;
            if (talent.workStyle && employer.workStyle) {
                checks++;
                if (talent.workStyle.toLowerCase() === employer.workStyle.toLowerCase())
                    score += 0.2;
                else
                    score -= 0.1;
            }
            ...
            if (talent.equityPreference) {
                ...
                if (pref === 'equity_heavy' && ['PRE_SEED', 'SEED'].includes(stage))
                    score += 0.15;
                else if (pref === 'cash_heavy' && ['SERIES_B', 'SERIES_C_PLUS', 'GROWTH'].includes(stage))
                    score += 0.15;
    ...
    scoreStageMatch(a, b) {
        if (!a.companyStage || !b.companyStage)
            return 0.5;
